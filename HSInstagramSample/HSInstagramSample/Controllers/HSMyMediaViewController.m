...
{
    [HSInstagramUser getUserWithId:@"self" withAccessToken:self.accessToken block:^(NSObject *obj) {
        HSInstagramUser * user = (HSInstagramUser * )obj;
        
        NSLog(@"User info: %@, %@, %@, %d, %d, %d", user.userName, user.userId, user.avatarUrl, user.posts, user.followers, user.followings);
...
- (void)buttonAction:(id)sender
{
    ...
    NSDictionary* params = [NSDictionary dictionaryWithObject:self.accessToken forKey:@"access_token"];
    NSString* path = [NSString stringWithFormat:kLikeEndpoint, [[self.images objectAtIndex:button.tag] imgId]];
    
    [[HSInstagram sharedClient] postPath:path
                             parameters:params
                                success:^(AFHTTPRequestOperation *operation, id responseObject) {
                                    NSLog(@"like photo successfully");
